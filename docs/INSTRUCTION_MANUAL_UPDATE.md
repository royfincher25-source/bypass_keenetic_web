# Инструкция по ручному обновлению bypass_keenetic на роутере

## Проблема
Кнопка "Обновить" в веб-интерфейсе не работает из-за:
1. Неправильного указания репозитория (используется `bypass_keenetic-web` вместо `bypass_keenetic_web`)
...
4. **Скачайте обновленные файлы:**
   ...
   # Основные файлы
   curl -sL -o app.py "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/app.py"
   curl -sL -o routes.py "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/routes.py"
   curl -sL -o env_parser.py "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/env_parser.py"
   curl -sL -o requirements.txt "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/requirements.txt"
   curl -sL -o .env.example "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/.env.example"
   ...
   for file in __init__.py app_config.py utils.py services.py ipset_manager.py list_catalog.py dns_manager.py dns_monitor.py dns_resolver.py web_config.py; do
       curl -sL -o "$file" "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/core/$file"
   done
   cd ..
   ...
   for file in base.html login.html index.html keys.html bypass.html install.html stats.html service.html updates.html bypass_view.html bypass_add.html bypass_remove.html bypass_catalog.html key_generic.html backup.html dns_monitor.html logs.html; do
       curl -sL -o "$file" "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/templates/$file"
   done
   cd ..
   
   # Статика и скрипты
   mkdir -p static scripts resources/scripts
   curl -sL -o static/style.css "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/static/style.css"
   curl -sL -o scripts/script.sh "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/scripts/script.sh"
   ...
   for script in unblock_ipset.sh unblock_dnsmasq.sh unblock_update.sh 100-redirect.sh 100-ipset.sh 100-unblock-vpn.sh 100-unblock-vpn-v4.sh S99unblock S99web_ui keensnap.sh; do
       curl -sL -o "$script" "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/resources/scripts/$script"
       chmod 755 "$script"
   done
   cd ../..
   ...
   curl -sL -o resources/config/dnsmasq.conf "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/resources/config/dnsmasq.conf"
   curl -sL -o resources/config/crontab "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/src/web_ui/resources/config/crontab"
   
   # VERSION
   curl -sL -o VERSION "https://raw.githubusercontent.com/royfincher25-source/bypass_keenetic_web/master/VERSION"
   ```
...
- Репозиторий `bypass_keenetic_web` (с подчёркиванием) правильный, а не `bypass_keenetic-web` (с дефисом)
