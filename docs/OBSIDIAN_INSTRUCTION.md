...
# Bypass Keenetic Web — Полное руководство
...
**Bypass Keenetic Web** — веб-интерфейс для управления системой обхода блокировок `bypass_keenetic` на роутерах Keenetic. Веб-интерфейс заменяет Telegram-бота и предоставляет тот же функционал через браузер.
...
### 1.2 Основные возможности v1.1.0
...
| 📲 **Установка/удаление** | One-click установка/удаление bypass_keenetic |
...
| ⚙️ **Сервис** | Перезапуск, DNS Override, Memory Manager |
| 📡 **DNS Мониторинг** | Автопереключение DNS при отказе (cloudflored) |

### 1.3 Новые функции v1.1.0

> [!success] Ключевые улучшения
...
```bash
# Шаг 2: Установка bypass_keenetic
...
# PowerShell:
scp -r "H:\path\to\bypass_keenetic-web\src\web_ui\*" root@192.168.1.1:/opt/etc/web_ui/
...
scp -r "H:\path\to\bypass_keenetic-web\src\web_ui" root@192.168.1.1:/opt/etc/
...
| 📲 | Установка | Установка/удаление bypass_keenetic |
...
**Удаление (Remove):**
- Остановка сервисов
- Удаление файлов bypass_keenetic
...
### 5.6 Управление bypass_keenetic
...
| Issues | https://github.com/royfincher25-source/bypass_keenetic_web/issues |
...
```
bypass_keenetic-web/              # Корень репозитория
...
- [[VERSION]] — Версия проекта (1.1.0)
- [[MIGRATION_TEST_TO_WEB]] — Миграция с Telegram-бота
- [GitHub Repository](https://github.com/royfincher25-source/bypass_keenetic_web)
- [Issues](https://github.com/royfincher25-source/bypass_keenetic_web/issues)
